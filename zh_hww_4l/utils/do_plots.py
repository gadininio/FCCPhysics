...
def runPlotsHistmaker(config: dict[str, Any],
    ...
    # Add signal scale factor to legend entry if signal is scaled (GADI)
    scale_sig = hist_cfg['scaleSig'] if 'scaleSig' in hist_cfg else 1.0
    for s in hsignal:
        leg.AddEntry(hsignal[s][0], param.legend[s].strip() + (f" (#times{scale_sig:g})" if scale_sig != 1.0 else ""), "l")
