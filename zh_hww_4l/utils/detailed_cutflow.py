...
def load_histogram(file_path, hist_name='cutFlow', hist_name_met='missingEnergy_final'):
    ...
    hist_met = file.Get(hist_name_met)
    if not hist:
        raise RuntimeError(f"Histogram {hist_name} not found in {file_path}")
        
    if not hist_met:
        print(f"Histogram {hist_name_met} not found in {file_path}")
        hist_met = None
        
    hist.SetDirectory(0)  # Detach from file
    if hist_met:
        hist_met.SetDirectory(0)  # Detach from file
        
    file.Close()
    return hist, hist_met
...
def fix_cut_name(cut):
    return cut.replace('{','').replace('}','').replace('#geq','≥').replace('#leq','≤').replace('#Delta','Δ').replace('#theta','θ').replace('#phi','φ').replace('#eta','η').replace('#gamma','γ').replace('#tau','τ').replace('#mu','μ').replace('^+','').replace('^-','').replace('#rightarrow','→')
...
parser.add_argument('--training', '-t', action='store_true', help='Use training samples.', default=False)
...
procs = config.get('training_processes' if args.training else 'processes', {})
if 'signal' not in procs or 'backgrounds' not in procs:
    raise ValueError(f"JSON must contain {'training_processes' if args.training else 'processes'} with 'signal' and 'backgrounds' definitions.")
...
hists = {}
hists_met = {} # Dictionary for the final variable
...
for sample_name in proc_list:
    ...
    if type(sample_file) == list:
        ...
        hists_met[sample_name] = None
        for sf in sample_file:
            hist, h_met = load_histogram(os.path.join(input_dir, f"{sf}.root"))
            if hists[sample_name] is None:
                hists[sample_name] = hist
                hists_met[sample_name] = h_met
            else:
                hists[sample_name].Add(hist)
                if h_met: hists_met[sample_name].Add(h_met)
    else:
        hist, h_met = load_histogram(os.path.join(input_dir, f"{sample_file}.root"))
        hists[sample_name] = hist
        hists_met[sample_name] = h_met
...
print("Combining signal histograms...")
...
hists_met[signal_combined_name] = combine_histograms(hists_met, signal_keys, combined_name='combined_signal_met')
proc_list = [signal_combined_name] + proc_list

# Combine background histograms into one for total background
print("Combining background histograms...")
...
# ---------------------------------------------------------
print(f"Generating cutflow table in {output_dir}...")
out_orig = sys.stdout
...
with open(output_path, 'w') as f:
    ...
    if add_perc:
        ...
        for i, cut in enumerate(cuts):
            ...
            if debug:
                print(f"Debug: Cut {i} - {cut}")
                print(f"  Signal (s): {s:.3f}")
                print(f"  Background (b): {b:.3f}")
                print(f"  S+B: {s+b:.3f}")
                print(f"  s_plus_b: {s_plus_b:.3f}")
        ...
        # --- NEW ENTRY ROW (Detailed Mode) ---
        entries_row = ["Entries", "", ""]
        entries_row += [f"{int(hists_met[p].GetEntries())}" for p in proc_list]
        print(formatted_row.format(*entries_row))
        # if export_latex: latex_file.write("\\hline\n" + to_latex_row(entries_row) + "\n")
