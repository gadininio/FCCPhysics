    ...
    for var, props in variables.items():
        ...
        for dist_key, dist_hists, dist_title in distributions:
            ...
            if ymax is not None:
                ...
                y_scale = 1+0.2+(0.1 if label != '' else 0)+0.005*len(dist_hists)  # Scale based on number of schemes to avoid legend overlap
            ...
            if label == '':
                ...
                tex.DrawLatex(latex_x, latex_y-0.10, label)
                tex.DrawLatex(latex_x, latex_y-0.15, dist_title)
            ...
            leg = ROOT.TLegend(0.7, latex_y-0.02, 0.88, latex_y-0.02-0.04*len(dist_hists))
            leg.SetBorderSize(0)
            leg.SetFillStyle(0)
