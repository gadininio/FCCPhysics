...
def plot_efficiencies():
    ...
    for var, var_dict in variables_to_plot.items():
        ...
        if h_num_total is not None and h_den_total is not None:
            ...
            leg = ROOT.TLegend(0.15, 0.55, 0.5, 0.7)
            leg.SetBorderSize(0)
            leg.SetFillStyle(0)
            ...
            latex_y = 0.83
            latex.DrawLatex(0.15, latex_y, "#bf{FCC-ee} IDEA Simulation (Delphes)")
            latex.DrawLatex(0.15, latex_y-0.05, f"#sqrt{{s}} = {ecm} GeV, L = {lumi} " + "ab^{-1}")
            latex.DrawLatex(0.15, latex_y-0.10, "e^{+}e^{-} #rightarrow Z(ll) H#left[W(l#nu)W(l#nu)#right]")
